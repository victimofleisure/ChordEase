...
/*
		...
		20		19nov13	in CreateCols, create row dialog via CreateRow
...
bool CRowView::CreateCols(int Cols, const COLINFO *ColInfo)
{
	if (Cols <= 0 || m_Cols)
		return(FALSE);
	// create dummy row before setting m_Cols, so CreateRow method can
	// easily identify this case if needed, by testing for m_Cols == 0
	CRowDlg	*rp = CreateRow(0);	// create dummy row dialog
	ASSERT(rp != NULL);
	m_ColState.SetSize(Cols);
	m_Cols = Cols;
	m_ColInfo = ColInfo;
	CRect	cr, dr;
	rp->GetWindowRect(dr);	// get row dialog's rectangle
	ScreenToClient(dr);
	...
	for (int iCol = 0; iCol <= Cols; iCol++) {
		if (iCol < Cols) {	// if not last loop
			CWnd	*RowCtrl = rp->GetDlgItem(ColInfo[iCol].CtrlID);
			...
			rp->ScreenToClient(cr);	// 14may11 added rp
			...
			&& !_tcsicmp(szClassName, _T("Edit"))) {
				...
				&& !_tcsicmp(szClassName, _T("msctls_updown32"))) {
					// only right-aligned spin controls are supported
					ASSERT(NextCtrl->GetStyle() & UDS_ALIGNRIGHT);
		...
		if (iCol > 0) {	// if not first column
			...
			CString	s((LPCTSTR)ColInfo[iPrevCol].TitleID);
...
int CRowView::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
    ...
    if (!m_Form->Create(NULL, NULL, dwStyle, r, this, 0, NULL))
		return -1;
